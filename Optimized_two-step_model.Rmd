...
lep <-lep %>% 
  select(temp_mean, temp_variance, YEAR, humi_mean, humi_variance,
...
lep <- lep %>% 
  select(-RegAgriHH,-Population,-PROV_AREA, -forest_area)
  ...
  foreach(param_c_idx = 1:nrow(param_grid_class), .packages = c("foreach", "xgboost")) %do% {
    ...
    foreach(param_r_idx = 1:nrow(param_grid_reg), .packages = c("foreach", "xgboost")) %do% {
      ...
      foreach(j = seq_along(inner_kfolds), .packages = "xgboost") %do% {
        ...
        # validation
        cols_to_remove <- c("label", "caseper100k", "AMP", "PROV", "YEAR")
        ...
        cols_to_remove <- intersect(colnames(val_reg),c("label", "caseper100k",
                                                        "AMP", "PROV", "YEAR"))
...
foreach(i = seq_along(kfolds_outer), .packages = c("foreach", "xgboost")) %do% {
  ...
  final_model_classification <- xgboost(
    data = as.matrix(train_set[, -c("label", "caseper100k",
                                    "AMP", "PROV", "YEAR")]),
  ...
  train_reg <- as.matrix(train_set_reg[, -c("label", "caseper100k",
                                        "AMP", "PROV", "YEAR")])
  # test set (regression)
...
result_df
write.csv(result_df, "result2.csv", row.names = FALSE)

result3 <- read.csv("actual_pred_3.csv")
conf_matrix_list
rmse <- sqrt(mean((result3$Pred - result3$Actual)^2))
mae_f <- mean(abs(result3$Pred - result3$Actual))
stopCluster(cl)
```
...
importance_matrix <- xgb.importance(
  ...
  feature_names = colnames(train_set[, -c("label", "caseper100k", "AMP", "PROV", "YEAR")])
)
...
result_mae <- result3 %>%
  group_by(PROV, AMP) %>%
  summarise(MAE = mean(abs(Pred - caseper100k), na.rm = TRUE))

result <- result %>% 
  select(Actual, Pred, PROV, AMP, YEAR, MONTH)
result_m <- result %>%
  filter(Actual > 0) %>%  # กรองเฉพาะค่าที่ Actual > 0
  group_by(YEAR, MONTH) %>%  # จัดกลุ่มตามปีและเดือน
  summarise(Count = n(), .groups = "drop") %>%  # นับจำนวนแถวที่ Actual > 0
  arrange(desc(Count))
...
result_summary <- result3 %>%
  group_by(PROV, AMP) %>%
  summarise(
    Actual_mean = mean(caseper100k, na.rm = TRUE),
    Pred_mean = mean(Pred, na.rm = TRUE),
    .groups = "drop"  # ใช้เพื่อลบการจัดกลุ่มออก
  )
write.csv(result_summary, "resultmean.csv", row.names = FALSE)

unique(result_summary$AMP)
...
result_summary$Actual_mean <- result_summary$Actual_mean + 0.0000001
result_summary$Pred_mean <- result_summary$Pred_mean- 0.0000001

summary(result_summary$Pred_mean)
